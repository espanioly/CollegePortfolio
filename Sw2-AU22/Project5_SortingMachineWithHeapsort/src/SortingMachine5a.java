...
public class SortingMachine5a<T> extends SortingMachineSecondary<T> {
    ...
    private static <T> void exchangeEntries(T[] array, int i, int j) {
        ...
        /*
         * Swap values at index i with the value at index j in the array
         */
        if (i != j) {
            T temp = array[i];
            array[i] = array[j];
    ...
    private static <T> void siftDown(T[] array, int top, int last,
         ...
         */
        /*
         * index of the top's left child
         */
        int leftPos = (2 * top) + 1;
        /*
         * index of the location of where the smallest value should be
         */
        int smallestPos = top;
        /*
         * check if top has a left child. if left == last then only one child.
         * if left < last then there are two children.
         */
        if (leftPos <= last) {
            /*
             * compare left child with smallestPos, updating the smallestPos iff
             * left < smallestPos
             */
            if (order.compare(array[leftPos], array[smallestPos]) < 0) {
                smallestPos = leftPos;
            }
            /*
             * index of top's right child
             */
            ...
            if (rightPos <= last) {
                /*
                 * Compare the right child with the left child, updating the
                 * smallestPos iff right < left
                 */
                if (order.compare(array[rightPos], array[smallestPos]) < 0) {
                    smallestPos = rightPos;
                }
            }

            /*
             * if the index of top and smallest are not the same then exchange
             * the entries so smallestPos is at the index of top and top is now
             * at the index of smallestPos
             */
            if (top != smallestPos) {
                exchangeEntries(array, top, smallestPos);
                siftDown(array, smallestPos, last, order);
            }
        }
    ...
    private static <T> void heapify(T[] array, int top, Comparator<T> order) {
        ...
        /*
         * Index of leftSide in array
         */
        int leftSide = (2 * top) + 1;
        /*
         * Index of the final value in the array
         */
        int last = array.length - 1;
        /**
         * Check if top has a left child. if leftSide == last then there is one
         * child. If leftSide < last there are two children.
         */
        if (leftSide <= last) {
            // heapify left side
            heapify(array, leftSide, order);
            /*
             * If the left side exists, the right may exist as well.
             */
            int rightSide = leftSide + 1;
            if (rightSide <= last) {
                // heapify right side
                heapify(array, rightSide, order);
            }
        }
        /*
         * Root may be out of place. Sift it down.
         */
    ...
    private static <T> T[] buildHeap(Queue<T> q, Comparator<T> order) {
        ...
        // Add values from q into heap
        for (int i = 0; i < heap.length; i++) {
            heap[i] = q.dequeue();
        }
        /*
         * Construct the heap from the given array start at the top (index 0)
         * following the total preorder given by order.
         */
    ...
    public final void add(T x) {
        ...
        // enqueue x to the end of entries
    ...
    public final void changeToExtractionMode() {
        ...
        // update this.heapSize because buldHeap clears this.entries
    ...
    public final T removeFirst() {
        ...
        // get the root
        T first = this.heap[0];
        // copy entry at the end and place it at the root
        this.heap[0] = this.heap[this.heapSize - 1];
        // reduce the size of the heap
        this.heapSize--;
        // siftDown the new root
    ...
    public final int size() {
        ...
        int size = 0;
        /*
         * If the sortingMachine is in insertion mode return the length of the
         * queue, else the sorting machine is in extraction mode then return the
         * size of the heap
         */
        if (this.insertionMode) {
