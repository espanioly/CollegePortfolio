...
public final class StringReassembly {
    ...
    public static String combination(String str1, String str2, int overlap) {
        ...
        //use substring to take the overlapping point then add the two strings together
        //use overlap

        String combined = (str1.substring(0, str1.length() - overlap))
                .concat(str2);
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet,
        ...
        //check if the string is a subset of one in the set
        int index = 0;
        Set<String> tempSet = strSet.newInstance();
        boolean isSubstring = false;
        while (strSet.size() > 0) {
            String tempString = strSet.removeAny();
            //if any current strings in the set are substrings of str, dont add
            if (!(str.indexOf(tempString) > -1)) {
                tempSet.add(tempString);
            }
            if (tempString.indexOf(str) > -1) {
                isSubstring = true;
            }
            //need to add a case where str and tempString are equal
            if (tempString.equals(str)) {
                tempSet.add(tempString);
            }

        }
        //if str was not a substring of any, add it to the set
        if (!isSubstring) {
            tempSet.add(str);
        }
    ...
    public static Set<String> linesFromInput(SimpleReader input) {
        ...
        Set<String> inp = new Set2<>();
        // input.atEOS();
        while (!input.atEOS()) {
            //take string put it in the sets
            // create set make while loop take string addtoavoidsubtring method
            addToSetAvoidingSubstrings(inp, input.nextLine());
        }
        return inp;
    ...
    public static void printWithLineSeparators(String text, SimpleWriter out) {
        ...
        // replace ~ with new line  aka out.print("\n")
