...
public final class CryptoUtilities {
    ...
    public static void reduceToGCD(NaturalNumber n, NaturalNumber m) {
         ...
         */
        if (!m.isZero()) { // checks m is not zero because we can't divide by 0
            NaturalNumber mod = (n.divide(m)); // remainder of the dividing
            reduceToGCD(m, mod); // recursive to find the common denominator
            n.transferFrom(m); // clears m and updates and n has the greatest denominator
        }
    ...
    public static void powerMod(NaturalNumber n, NaturalNumber p,
         ...
         */
        // get original n
        NaturalNumber origin = new NaturalNumber2(n);
        NaturalNumber two = new NaturalNumber2(2);
        if (!p.isZero()) { // checks if power is zero
            boolean isEven = (isEven(p)); // checks if power is even
            p.divide(two);// divide power by 2
            powerMod(n, p, m); //recursive
            // restoring p
            p.multiply(two);
            if (!isEven) {
                p.increment();
            ...
            //
            NaturalNumber copyN = new NaturalNumber2(n); // copy of new N
            n.multiply(copyN);
            NaturalNumber remain = n.divide(m); // mod m
            n.transferFrom(remain);
            if (!isEven) { // if p is odd then n multiplied with its original n
                n.multiply(origin); // to get the last p
                remain = n.divide(m); // mod m
                n.transferFrom(remain);// n is the new remainder
            }
        } else {
            n.setFromInt(1);
        } // else it returns 0
...
//        assert (new NaturalNumber2(1)).compareTo(w) < 0 : "Violation of: 1 < w";
        ...
        n.increment();
        //making sure the values are restored
        boolean isTrueOrFlase = false;
        NaturalNumber temoN = new NaturalNumber2(n);
        NaturalNumber two = new NaturalNumber2(2);
    ...
    public static boolean isPrime2(NaturalNumber n) {
        assert n.compareTo(new NaturalNumber2(1)) > 0 : "Violation of: n > 1";
        // similar as prime1 instead we loop it 50 times with n and
        //make sure to use isWitnessToCompositeness
        ...
        //making sure that both methods work in here
    ...
    public static void generateNextLikelyPrime(NaturalNumber n) {
        assert n.compareTo(new NaturalNumber2(1)) > 0 : "Violation of: n > 1";
        // use loop until n is a prime (prime2 is true)
